...
ui <- dashboardPage(skin = "black",
             ...
             tabPanel("Averaged Data",
                      ...
                      actionButton("exclude_reset4", "Reset Clicked Points"),
                      br(),
                      br(),
                      fluidRow(
                      column(1, 
...
output$meanPlot1 <- renderPlot({
  ...
  excludeAv <-excludeAv[do.call(order, excludeAv[as.character(xVar)]), ]
...
selectData <- reactive({
  ...
  selectedAv <-selectedAv[do.call(order, selectedAv[as.character(xVar)]), ]
  if(input$noStages > 0){
  ...
  selectedAv <- selectedAv[rowSums(is.na(selectedAv))!=ncol(selectedAv), ]
  selectedAv <-selectedAv[,c(1:2,ncol(selectedAv), 3:(ncol(selectedAv)-1)) ]
})
